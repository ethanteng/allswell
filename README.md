...
> heuristic analyzer. See [How the analysis works](#how-the-analysis-works).
...
it the app still works: `heuristic-analyzer.ts` parses the transcript and cites
...
Use a **workspace-scoped** key. An organization-level key has no workspace of
...
and key revocation land on this app rather than on everything the organization
runs.
...
**The model only writes judgment.** The schema it fills in
...
- **No `ANTHROPIC_API_KEY`** → the heuristic analyzer, labeled a placeholder
...
in the placeholder analyzer, which only emits a card when it has lines to cite.
...
a session re-analyzes it under the new one. Admin is gated by `isAdmin`, set
...
- Scoping is already per-clinician. Supervisor access across clinicians is the
  first real authorization problem, and it's a genuine one.
